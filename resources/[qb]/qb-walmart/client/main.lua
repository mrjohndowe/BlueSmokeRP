    ...
    CreateThread(function()
        exports['qb-target']:AddBoxZone('PawnShop', Config.PawnLocation.coords, Config.PawnLocation.length, Config.PawnLocation.width, {
            ...
            options = {
                {
                    type = 'client',
                    event = 'qb-walmart:client:openMenu',
                    icon = 'fas fa-ring',
                    label = 'Walmart Buy Back',
                },
            },
...
RegisterNetEvent('qb-walmart:client:openMenu', function()
        ...
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed then
            local pawnShop = {
                ...
                },
                {
                    ...
                    params = {
                        event = 'qb-walmart:client:openPawn',
                        args = {
            ...
            -- if not isMelting then
                -- pawnShop[#pawnShop + 1] = {
                    ...
                    -- params = {
                        -- event = 'qb-walmart:client:openMelt',
                        -- args = {
            ...
            -- if canTake then
                -- pawnShop[#pawnShop + 1] = {
                    ...
                    -- params = {
                        -- isServer = true,
                        -- event = 'qb-walmart:server:pickupMelted',
                        -- args = {
                            -- items = meltedItem
                        -- }
                    -- }
...
RegisterNetEvent('qb-walmart:client:openPawn', function(data)
    QBCore.Functions.TriggerCallback('qb-walmart:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    pawnMenu[#pawnMenu + 1] = {
                        ...
                        params = {
                            event = 'qb-walmart:client:pawnitems',
                            args = {
...
-- RegisterNetEvent('qb-walmart:client:openMelt', function(data)
    -- QBCore.Functions.TriggerCallback('qb-walmart:server:getInv', function(inventory)
        -- local PlyInv = inventory
        -- local meltMenu = {
            -- {
            ...
            -- for i = 1, #data.items do
                -- if v.name == data.items[i].item then
                    -- meltMenu[#meltMenu + 1] = {
                        ...
                        -- params = {
                            -- event = 'qb-walmart:client:meltItems',
                            -- args = {
                                -- label = QBCore.Shared.Items[v.name].label,
...
RegisterNetEvent('qb-walmart:client:pawnitems', function(item)
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('qb-walmart:server:sellPawnItems', item.name, sellingItem.amount, item.price)
        else
...
-- RegisterNetEvent('qb-walmart:client:meltItems', function(item)
    ...
    -- if meltingItem then
        ...
        -- if meltingItem.amount ~= nil then
            -- if tonumber(meltingItem.amount) > 0 then
                -- TriggerServerEvent('qb-walmart:server:meltItemRemove', item.name, meltingItem.amount, item)
...
-- RegisterNetEvent('qb-walmart:client:startMelting', function(item, meltingAmount, meltTimees)
    -- if not isMelting then
        -- isMelting = true
        -- meltTime = meltTimees
        -- meltedItem = {}
...
RegisterNetEvent('qb-walmart:client:resetPickup', function()
    canTake = false
end)
