...
local function exploitBan(id, reason)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'pawnshop', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'qb-walmart', reason), true)
    DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end

RegisterNetEvent('qb-walmart:server:sellPawnItems', function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    TriggerClientEvent('qb-walmart:client:openMenu', src)
end)

RegisterNetEvent('qb-walmart:server:meltItemRemove', function(itemName, itemAmount, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], 'remove')
        local meltTime = (tonumber(itemAmount) * item.time)
        TriggerClientEvent('qb-walmart:client:startMelting', src, item, tonumber(itemAmount), (meltTime * 60000 / 1000))
...
RegisterNetEvent('qb-walmart:server:pickupMelted', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local playerCoords = GetEntityCoords(GetPlayerPed(src))
    local dist = #(playerCoords - Config.PawnLocation.coords)
    ...
    TriggerClientEvent('qb-walmart:client:resetPickup', src)
    TriggerClientEvent('qb-walmart:client:openMenu', src)
end)

QBCore.Functions.CreateCallback('qb-walmart:server:getInv', function(source, cb)
