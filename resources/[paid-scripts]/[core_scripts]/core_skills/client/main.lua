...
Citizen.CreateThread(function()
    while true do
        ...
        if display == false then
            if IsControlJustReleased(0, Config.KeyToOpenSkillMenu) then
                SetDisplay(true)
            end
        else
            Citizen.Wait(1000)
                                ...
                                QBCore.Functions.TriggerCallback("core_skills:getAbilities", function(data)
                                    for k,v in pairs(data) do
                                        if v.skill_name == "AddHealthRegen" then
                                            ...
                                            TriggerServerEvent('core_skills:saveAbilities', e, StartHPRegeneration, StopHPRegeneration, HealthRegenTime)
                                        end
                                    end
                                end)
                            else
                                ...
                                QBCore.Functions.TriggerCallback("core_skills:getAbilities", function(data)
                                    for k,v in pairs(data) do
                                        if v.skill_name == "AddArmorRegen" then
                                            ...
                                            TriggerServerEvent('core_skills:saveAbilities', e, StartArmorRegeneration, StopArmorRegeneration, ArmorRegenTime)
                                        end
                                    end
                                end)
                            else
...
Citizen.CreateThread(function()
    while true do
        ...
        if StartAddArmorTimer == true then
...
Citizen.CreateThread(function()
    while true do
        ...
        if HaveAddDrivingSpeed == true then
            local ped = PlayerPedId()
                    ...
                    if IsControlPressed(0, 21) and IsControlJustReleased(0, 137) then
                        ...
                        if GetVehiclePedIsIn(ped, false) == 0 then
                            if not IsPedSwimming(ped) then 
                                ...
                                while not HasModelLoaded("prop_tyre_wall_01b") or not HasCollisionForModelLoaded("prop_tyre_wall_01b") do
                                    Wait(1)
                                end		

                                if pedheading >= 260 then 
                                
                                    wall = CreateObject(GetHashKey("prop_tyre_wall_01b"), pedcoords[1]+1.5, pedcoords[2]+1.5, pedcoords[3], true, false, true)--object example
                                ...
                                elseif pedheading >= 100 and pedheading < 160 then
                                    ...
                                    wall = CreateObject(GetHashKey("prop_tyre_wall_01b"), pedcoords[1]-1.5, pedcoords[2], pedcoords[3], true, false, true)--object example
...
Citizen.CreateThread(function() 
    while true do
        ...
        if HaveAddStaminaSprintTime == true then
            local ped = PlayerPedId()
            local pedId = PlayerId()
...
Citizen.CreateThread(function() 
    while true do
        ...
        if HaveAddStaminaRecoveryTime == true then
            local ped = PlayerPedId()
            local pedId = PlayerId()
            local Stamina = math.floor(100 - GetPlayerSprintStaminaRemaining(pedId))
