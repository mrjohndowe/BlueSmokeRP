							...
							if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
								...
								Citizen.CreateThread(
									function()
                                        ...
                                        TaskPlayAnim(PlayerPedId(), "mini@repair", "fixing_a_ped" ,3.0, 3.0, -1, 25, 0, false, false, false)
							...
							if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
								...
								Citizen.CreateThread(
									function()
										...
										TaskStartScenarioInPlace(ped, "WORLD_HUMAN_VEHICLE_MECHANIC", 0, true) -- amb@prop_human_movie_bulb@base  PROP_HUMAN_BUM_BIN
    ...
    function(tool)
        ...
        if tool == "mechanic_tools" then
            if hasJob() then
                ...
                local dst = GetDistanceBetweenCoords(GetEntityCoords(veh), coords, true)
				local plate = GetVehicleNumberPlateText(veh)
...
RegisterNetEvent('buddy_vehfix')
AddEventHandler('buddy_vehfix', function()
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
        if GetDistanceBetweenCoords( 450.8977, -976.0216, 25.699796, GetEntityCoords(PlayerPedId())) < 5.0 then
        ...
        SendTextMessage(Config.Text["mechanic_action_started"])
        repairMode = true
        repairPartType = "engine"
        SetEntityCoordsNoOffset(Vehicle, 450.8977, -976.0216, 25.699796)
