...
AddEventHandler( "core_vehicle:canInstall", function(partType, part)
		...
		if xItem ~= nil then
			if xItem.amount > 0 then
				...
				TriggerClientEvent("QBCore:Notify", src, Config.Text["not_enough"])
			end
		else
	            TriggerClientEvent("QBCore:Notify", src, Config.Text["not_enough"])
		end
            ...
            function(parts)
                ...
                if parts[1] ~= nil then
                    ...
                    exports.oxmysql:update(
                        "REPLACE INTO vehicle_parts (plate, parts) values(?,?)",
                        {plate,json.encode(defaultParts)},
                        function()
                        end
                    )
...
QBCore.Functions.CreateCallback('core_vehicle:getIfVehicleOwned',function(source, cb, plate)
    ...
    exports.oxmysql:query("SELECT * FROM player_vehicles WHERE plate=?",{plate}, function(data) 
