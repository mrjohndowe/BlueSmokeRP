...
RegisterNetEvent(CorePrefix .. ':Client:OnPlayerLoaded')
AddEventHandler(CorePrefix .. ':Client:OnPlayerLoaded', function()
...
AddEventHandler('bb-garages:client:releaseVehicle', function(data, typ, name)
    ...
    if typ ~= 'impounds' then
        for k, v in pairs(nearbyVehicles) do
            ...
            if DoesEntityExist(v) then
                if GetVehicleNumberPlateText(v) == data.plate then
                    ...
                    TriggerEvent(CorePrefix .. ':Notify', "Successfully released your vehicle [Slot " .. parking[1] .. ']', "success")
        ...
        if not released then
            ...
            if BBGarages.Functions.IsSpawnClear(BBGarages.Config['garages'][parking[2]]['slots'][tonumber(parking[1])][1], 2.0) then
                ...
                TriggerEvent(CorePrefix .. ':Notify', "Someone is parking on your slot! [Slot " .. parking[1] .. ' Isn\'t Free]', "error")
            end
        end
    elseif typ == 'impounds' then
        RLCore.Functions.SpawnVehicle(data.model, function(vehicle)
            ...
            table.insert(ownedVehicles, {GetVehicleNumberPlateText(vehicle), vehicle})
            TriggerEvent("vehiclekeys:client:SetOwner", data.plate, vehicle)
            TriggerEvent(CorePrefix .. ':Notify', "Successfully released your vehicle", "success")
...
AddEventHandler('bb-garages:client:fakeplate:steal', function(data)
    ...
    if dst <= 7.0 then
        if plate:gsub(" ", "") ~= '' and plate:gsub(" ", "") ~= '       ' then
            ...
            TriggerEvent(CorePrefix .. ':Notify', "This vehicle doesn\'t have license plate on, what are you trying to do bruh", "error")
        end
    else
...
AddEventHandler('bb-garages:client:fakeplate:usePlate', function(data)
    ...
    if dst <= 7.0 then
        if plate == '' or plate == '        ' then
            ...
            TriggerEvent(CorePrefix .. ':Notify', "This vehicle already has license plate on, remove it first", "error")
        end
    else
        ClearPedTasksImmediately(PlayerPedId())
    end
...
RegisterNUICallback('waypoint', function(data)
    ...
    if typ == 'garage' then
        ...
        if garage ~= nil then
            ...
            TriggerEvent(CorePrefix .. ':Notify', "Updated closest garage waypoint on your GPS", "success")
        else
            print('[bb-garages] Error while setting garage waypoint')
            TriggerEvent(CorePrefix .. ':Notify', "Error while setting garage waypoint", "error")
        end
    elseif typ == 'impound' then
        ...
        TriggerEvent(CorePrefix .. ':Notify', "Updated closest impound waypoint on your GPS", "success")
    end                      
end)
...
RegisterNUICallback('vehicleblip', function(data)
    ...
    for _, vehicle in pairs(ownedVehicles) do
        if vehicle[1] == plate then
            ...
            TriggerEvent(CorePrefix .. ':Notify', "Updated your vehicle on the GPS", "success")
            return
        end
    end

    if found == false then
        TriggerEvent(CorePrefix .. ':Notify', "Could not find your vehicle, it placed on the impound", "error")
...
RegisterNUICallback('payout', function(data)
    ...
    if typ == 'impounds' then
        if BBGarages.Functions.IsSpawnClear(BBGarages.Config['impounds'][garage]['spawn'], 2.0) then
            ...
            TriggerEvent(CorePrefix .. ':Notify', "The spawnpoint isn\'t free, check it out", "error")
        end
    else
        TriggerServerEvent('bb-garages:server:vehiclePayout', garage, plate, price, typ) 
    end         
