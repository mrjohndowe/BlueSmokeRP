main = {
    ...
    -- Add your webhook in sv_averagespeed.lua
    ...
    useKmh = false,
    exemptVehicleClasses = {18}, -- Vehicle classes that are exempt from camera logging, use 18 for emergency, example = {18, 20} emergency + commercial
    exemptVehicleNames = {`police`, `police2`}, -- Vehicle names that are exempt from camera logging, eg, {`police`, `police2`} using a comma to separate
    flashScreen = true,
    modelHash = `prop_speed_camera`,
    reduceBy = 0.85,
    enableBlips = false,
    shortRangeBlips = false,
    blipScale = 0.7,
    notifications = { -- Only set one to true
        mythicNotify = false,
        ESX = false,
        QBCore = true,
        vRP = false,
...
translations = {
    ...
    fined = "You have been fined ",
    currency = "$",
    blipStart = "Camera 1",
    blipEnd = "Camera 2",
}

framework = { -- Enable only the one that you need for your framework.
    ESX = {
        enabled = false,
        ESXBilling = { -- support for https://github.com/kompots/esx_billing
            enabled = false,
            society = "",
            name = "",
            amount = 100,
