			...
			if not menuactive and distance <= 20.0 then
				...
				if distance <= 1.0 then
					...
					if IsControlJustPressed(0,38) then
						empresaAtual = k
						TriggerServerEvent("vrp_stores:getData",empresaAtual) 
					end
				end
			end
			...
			for _,mark in pairs(v.sell_blip_coords) do
				...
				if distance <= 20.0 then
					...
					if distance <= 1.0 then
						...
						if IsControlJustPressed(0,38) then
							empresaAtual = k
							TriggerServerEvent("vrp_stores:openMarket",k) 
						end
					end
				end
			end
			...
			if distance <= 20.0 then
				...
				if distance <= 1.0 then
					if job_data[k] == nil then
						...
						if IsControlJustPressed(0,38) then
							TriggerServerEvent('vrp_stores:getJob',k)
						end
					else
						DrawText3D2(x,y,z-0.6, Lang[Config.lang]['show_jobs']:format(job_data[k].name,job_data[k].reward), 0.40)
						if IsControlJustPressed(0,38) then
							if truck then
								TriggerEvent("stores:Notify","negado",Lang[Config.lang]['already_has_job'])
								break
							end
							...
							if checkPos == false then
								TriggerEvent("stores:Notify","negado",Lang[Config.lang]['occupied_places'])
								break
							end
							empresaAtual = k
							TriggerServerEvent('vrp_stores:startJob',k,job_data[k].id)
						end
					end
				else
					job_data[k] = nil
...
RegisterNetEvent('vrp_stores:getJob')
AddEventHandler('vrp_stores:getJob', function(k,data)
	job_data[k] = data
end)

RegisterNetEvent('vrp_stores:open')
AddEventHandler('vrp_stores:open', function(dados,update,isMarket)
	-- Abre NUI
	SendNUIMessage({ 
		showmenu = true,
		update = update,
...
RegisterNetEvent('vrp_stores:openRequest')
AddEventHandler('vrp_stores:openRequest', function(price)
...
RegisterNUICallback('startJob', function(data, cb)
	if cooldown0 == nil then
		...
		TriggerServerEvent('vrp_stores:startContract',empresaAtual,data.item_id,data.amount)
...
RegisterNUICallback('startJob2', function(data, cb)
	if cooldown0 == nil then
		cooldown0 = true
		...
		TriggerServerEvent('vrp_stores:startContract2',empresaAtual,data.item_id)
...
RegisterNUICallback('storeProduct', function(data, cb)
	if cooldown0 == nil then
		cooldown0 = true

		TriggerServerEvent('vrp_stores:storeProduct',empresaAtual,data.item_id,data.amount)
...
RegisterNUICallback('setPrice', function(data, cb)
	TriggerServerEvent('vrp_stores:setPrice',empresaAtual,data.item_id,data.price)
end)

RegisterNUICallback('createJob', function(data, cb)
	TriggerServerEvent('vrp_stores:createJob',empresaAtual,data)
end)

RegisterNUICallback('renameMarket', function(data, cb)
	TriggerServerEvent('vrp_stores:renameMarket',empresaAtual,data)
end)

RegisterNUICallback('buyUpgrade', function(data, cb)
	TriggerServerEvent('vrp_stores:buyUpgrade',empresaAtual,data)
end)
...
RegisterNUICallback('deleteJob', function(data, cb)
	if cooldown1 == nil then
		...
		TriggerServerEvent('vrp_stores:deleteJob',empresaAtual,data)
...
RegisterNUICallback('depositMoney', function(data, cb)
	TriggerServerEvent('vrp_stores:depositMoney',empresaAtual,data)
end)
...
RegisterNUICallback('withdrawMoney', function(data, cb)
	if cooldown2 == nil then
		...
		TriggerServerEvent('vrp_stores:withdrawMoney',empresaAtual)
...
RegisterNUICallback('sellMarket', function(data, cb)
	if cooldown3 == nil then
		...
		closeUI()
		TriggerServerEvent('vrp_stores:sellMarket',key)
...
RegisterNUICallback('buyMarket', function(data, cb)
	if cooldown4 == nil then
		...
		local key = empresaAtual
		TriggerServerEvent('vrp_stores:buyMarket',key)
...
RegisterNUICallback('buyItem', function(data, cb)
	TriggerServerEvent('vrp_stores:buyItem',empresaAtual,data)
end)
...
RegisterNetEvent('vrp_stores:startContract')
AddEventHandler('vrp_stores:startContract', function(item,amount,truck_level,query_delivery)
	local key = empresaAtual
	job_data[key] = nil
	...
	TriggerEvent("stores:Notify","sucesso",Lang[Config.lang]['already_is_in_garage'])
	closeUI()
	...
	while truck do
				...
				if distance <= 2 then
					...
					if IsControlJustPressed(0,38) then
						if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true)) then
							...
							TriggerEvent("stores:Notify","sucesso",Lang[Config.lang]['bring_to_van'])
						else
							TriggerEvent("stores:Notify","negado",Lang[Config.lang]['out_of_veh'])
						end
					end
				end
			end
				...
				if distance <= 1.5 then
					...
					if IsControlJustPressed(0,38) then
						if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true))  then
							...
							TriggerEvent("stores:Notify","sucesso",Lang[Config.lang]['bring_to_store'])
		...
		elseif fase_coleta == 3 then
			...
			if distance <= 50 and veh == truck then
				...
				if distance <= 4 then
					...
					truck_blip = nil
					route_blip = nil
					TriggerServerEvent("vrp_stores:finishContract",key,item,amount,distance_traveled,query_delivery)
					return
				end
			end
		end
		...
		if not IsEntityAVehicle(truck) then
			...
			truck_blip = nil
			route_blip = nil
			TriggerServerEvent("vrp_stores:failed",query_delivery)
			return
		end
		...
		if IsEntityDead(ped) then
			...
			route_blip = nil
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("stores:Notify","negado",Lang[Config.lang]['you_died'])
			TriggerServerEvent("vrp_stores:failed",query_delivery)
			return
		end
		...
		if engineH <= 150 then
			...
			route_blip = nil
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("stores:Notify","negado",Lang[Config.lang]['vehicle_destroyed'])
			TriggerServerEvent("vrp_stores:failed",query_delivery)
			return
		end
...
RegisterNetEvent('vrp_stores:startContract2')
AddEventHandler('vrp_stores:startContract2', function(item,amount,truck_level)
	local key = empresaAtual
	job_data[key] = nil
	...
	local rand = math.random(#Config.export_locations)
	local x,y,z = table.unpack(Config.export_locations[rand])
	local route_blip = createBlip(x,y,z)
	...
	while truck do
		...
		if fase_coleta == 1 then
			...
			if distance <= 50 and veh == truck then
				...
				DrawMarker_truck(x,y,z)
				if distance <= 2 then
					DrawText3D2(x,y,z-0.6, Lang[Config.lang]['objective_marker_3'], 0.40)
					if IsControlJustPressed(0,38) then
						BringVehicleToHalt(truck, 2.5, 1, false)
						Citizen.Wait(10)
						DoScreenFadeOut(500)
		...
		elseif fase_coleta == 2 then
			...
			if distance <= 50 and veh == truck then
				...
				if distance <= 4 then
					...
					truck_blip = nil
					route_blip = nil
					TriggerServerEvent("vrp_stores:finishContract2",key,item,amount,distance_traveled)
					return
				end
			end
		end
...
function DeletarObjeto()
    ...
    if DoesEntityExist(object) then
        TriggerServerEvent("vrp_stores:trydeleteobj",ObjToNet(object))
        object = nil
    end
end

RegisterNetEvent("vrp_stores:syncdeleteobj")
AddEventHandler("vrp_stores:syncdeleteobj",function(index)
	...
	while true do
		if truck then
			...
			if IsControlJustPressed(0,Config.keyToUnlockTruck) then
				TriggerServerEvent("vrp_stores:vehicleLock")
			end
		end
		Citizen.Wait(timer)
	end
...
RegisterNetEvent('vrp_stores:vehicleClientLock')
AddEventHandler('vrp_stores:vehicleClientLock', function()
	local v = truck
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		if lock == 1 then
			...
			TriggerEvent("stores:Notify","importante",Lang[Config.lang]['vehicle_locked'],8000)
		else
			SetVehicleDoorsLocked(v,1)
			TriggerEvent("stores:Notify","importante",Lang[Config.lang]['vehicle_unlocked'],8000)
		end
...
Citizen.CreateThread(function()
	Wait(5000)
	TriggerServerEvent("vrp_stores:getBlips")
end)

RegisterNetEvent('vrp_stores:setBlips')
AddEventHandler('vrp_stores:setBlips', function(blips_table)
	for k,v in pairs(Config.market_locations) do
		local x,y,z = table.unpack(v.coord)
		...
		if blips_table[k] and blips_table[k].market_blip and blips_table[k].market_name and blips_table[k].market_color then
			market_blips[k] = addBlip(x,y,z,tonumber(blips_table[k].market_blip),tonumber(blips_table[k].market_color),blips_table[k].market_name,blips.scale)
		else
			market_blips[k] = addBlip(x,y,z,blips.id,blips.color,blips.name,blips.scale)
		end
	end
end)

RegisterNetEvent('vrp_stores:updateBlip')
AddEventHandler('vrp_stores:updateBlip', function(key,name,color,blip)
	RemoveBlip(market_blips[key])
	local x,y,z = table.unpack(Config.market_locations[key].coord)
	local blips = Config.market_types[Config.market_locations[key].type].blips
	market_blips[key] = addBlip(x,y,z,tonumber(blip),tonumber(color),name,blips.scale)
end)
